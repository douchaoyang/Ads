...
**@Relyed  core.lib.js
*/
window.Ads = window.Ads || function($) {
    var Ads = function(params) {
        ...
        this.ing = true;
        /*是否正在进行动画标记*/
        this.useHover = params.useHover || false;
        /*切换控制方式*/
        this.mouseOn = false;
        /*清除定时器标记*/
        this.container = g(params.container);
        /*广告容器*/
        this.containerChild = this.container.getElementsByTagName("a");
        /*广告必须是a标签*/
        this.control = g(params.control);
        /*控制按钮容器*/
        for (var i = 0; i < this.containerChild.length; i++) {
            var _span = $.creatElment("span");
            this.control.appendChild(_span);
        }
        this.controlChild = this.control.getElementsByTagName("span");
        /*控制按钮是span标签*/
    ...
    Ads.prototype.rollFn = function(n) {
        ...
        if (n > this.now) {
            for (var i = this.now + 1; i < n; i++) {
                this.containerChild[i].style.left = "-100%";
            }
            this.goLeft(this.containerChild[this.now], this.containerChild[n], n);
        } else {
            for (var i = n + 1; i < this.now; i++) {
                this.containerChild[i].style.left = "100%";
            }
            this.goRight(this.containerChild[this.now], this.containerChild[n], n);
        }
        for (var i = 0; i < this.controlChild.length; i++) {
            this.controlChild[i].className = "";
        }
        this.controlChild[n].className = this.onClass;
    };
        ...
        (function() {
            ...
            if (times > 0) {
                ...
                if (_this.autoRun === null && _this.mouseOn != true) {
                    _this.autoRun = setTimeout(function() {
                        _this.rollFn(_this.now + 1);
                        _this.stopFn();
                    }, _this.stay);
                }
                _this.ing = true;
